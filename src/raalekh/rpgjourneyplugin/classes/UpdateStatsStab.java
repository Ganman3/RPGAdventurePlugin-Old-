...
public class UpdateStatsStab {
	...
	public void setStats (Player player, int currentLevel) {
		...
		if(currentLevel <= 20) {
			...
			double attack = 6 + currentLevel * 0.8;
			...
			double luck = 2 + currentLevel * 0.8;
			try {
				...
				playerData.set("Nature", "Physical");
		...
		if(currentLevel > 20 && currentLevel <= 150) {
			...
			double health = cp.getHandle().getAttributeInstance(GenericAttributes.maxHealth).getValue();
			double attack = 30 + currentLevel * 0.4;
			...
			double luck = 10 + currentLevel * 0.4;
