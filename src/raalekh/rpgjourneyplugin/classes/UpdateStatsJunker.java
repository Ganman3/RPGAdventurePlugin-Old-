...
public class UpdateStatsJunker {
	private String name;
	...
	public void setStats (Player player, int currentLevel) {
		...
		if(currentLevel <= 20) {
			...
			double attack = 4 + currentLevel * 0.8;
			double defense = 4.5 + currentLevel * 0.8;
			double attunement = 2.5 + currentLevel * 0.8;
			double resistance = 3 + currentLevel * 0.8;
			double speed = 5 + currentLevel * 0.8;
			double diplomacy = 2 + currentLevel * 0.8;
		...
		if(currentLevel > 20 && currentLevel <= 150) {
			...
			double health = cp.getHandle().getAttributeInstance(GenericAttributes.maxHealth).getValue();
			double attack = 20 + currentLevel * 0.4;
			double defense = 22.5 + currentLevel * 0.4;
			double attunement = 12.5 + currentLevel * 0.4;
			double resistance = 15 + currentLevel * 0.4;
			double speed = 25 + currentLevel * 0.4;
			double diplomacy = 10 + currentLevel * 0.4;
