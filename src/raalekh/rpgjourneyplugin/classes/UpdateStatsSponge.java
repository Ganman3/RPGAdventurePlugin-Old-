...
public class UpdateStatsSponge {
	private String name;
	...
	public void setStats (Player player, int currentLevel) {
		...
		if(currentLevel <= 20) {
			...
			cp.getHandle().getAttributeInstance(GenericAttributes.maxHealth).setValue(8 + currentLevel * 0.4);
			double health = cp.getHandle().getAttributeInstance(GenericAttributes.maxHealth).getValue();
			double attack = 2 + currentLevel * 0.8;
			double defense = 4 + currentLevel * 0.8;
			double attunement = 2 + currentLevel * 0.8;
			double resistance = 4 + currentLevel * 0.8;
			double speed = 2 + currentLevel * 0.8;
			double diplomacy = 4 + currentLevel * 0.8;
			double dodge = 5 + currentLevel * 0.8;
			double absorption = 6 + currentLevel * 0.8;
		...
		if(currentLevel > 20 && currentLevel <= 150) {
			...
			double defense = 20 + currentLevel * 0.4;
			double attunement = 10 + currentLevel * 0.4;
			double resistance = 20 + currentLevel * 0.4;
			double speed = 10 + currentLevel * 0.4;
			double diplomacy = 20 + currentLevel * 0.4;
			double dodge = 25 + currentLevel * 0.4;
			double absorption = 30 + currentLevel * 0.4;
